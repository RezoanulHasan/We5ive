...
const WomenProducts: React.FC = () => {
  useTitle("WoMen Product");

  const [showAll, setShowAll] = useState(false);

  const womenProducts = React.useMemo(
    () => productDatas.filter((product) => product.gender === "Women"),
    []
  );

  const displayedProducts = showAll ? womenProducts : womenProducts.slice(0, 8); // Display 8 products initially
    ...
    <div className="p-8 mt-20">
      ...
      {womenProducts.length === 0 ? (
        <p className="text-gray-500 text-center">
          No products available for women at the moment.
        </p>
      ) : (
